# https://habr.com/ru/company/skillfactory/blog/549470/
import numpy as np
from pylab import *
...
for i in range(1000):
    if i == 0:
        BxSi_0.append(
            b_1 * Si_0[0] + b_2 * (Si_0[1]))  # V can be maipulated by the equation in Text book
    else:
...
def sifuntion(i):  # In next iteration, Last iteration Si_stepup will be this iteration's Si_stepdown
    if i == 0:
        ...
        V[1:N_Slices - 1] = b_1 * Si_stepdown[1:N_Slices - 1] + b_2 * (
                Si_stepdown[2:N_Slices] + Si_stepdown[0:N_Slices - 2])
